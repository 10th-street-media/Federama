 ...
 * pub/dash/inbox.php
 *
 * A page with all messages sent to an account.
 *
...
$pagetitle = _("Inbox « $u_name « Ꞙederama");
include "header.php";
include "nav.php";
?>
			...
			<article class="w3-padding w3-col s12 m8 l10">
				...
				<h2 class="w3-padding"><?php echo _("Inbox"); ?></h2>

<?php
/**
 * In the future, this page will only have messages that appear in an account's inbox
 */

/**
 * Let's see if there is anything in the inbox
 */
$inboxq = "SELECT * FROM ".TBLPREFIX."users WHERE user_name='".$u_name."'";
$inboxquery = mysqli_query($dbconn,$inboxq);
while ($inboxopt = mysqli_fetch_assoc($inboxquery)) {
	// turns a comma separated string into an array
	$inbox = explode(",",$inboxopt['user_inbox']);

	foreach ($inbox as $message) {

		// decode the url for this message
		if ($message !== '') {
			$messageurl = urldecode($message);

			// the message_id is also a url (not encoded)
		}
	}
}
/**
 ...
 */
 ...
 echo "\t\t\t\t<p>The <b>inbox</b> will contain every message sent to the user, whether public or private, as well as notifications when someone follows them, and when someone likes, dislikes, or shares one of their messages, posts, pages, or media.</p>\n";
 echo "\t\t\t\t<p>For now, the inbox concentrates on messages sent to the user.</p>\n";
 echo "\t\t\t\t</section>\n";
	...
	echo "\t\t\t\t<p class=\"w3-padding\">"._('Your inbox is currently empty.')."</p>";
