 ...
 * ATOM FEED
 */
	$atommeta = fopen($website_url."users/".$username."-atom.xml", "w") or die("Unable to open or create ".$website_url."users/".$username."-atom.xml file");

	$atomstart = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n<feed xmlns=\"http://www.w3.org/2005/Atom\">\n\n\t<title>".$website_name." : ".$username."</title>\n\t<link rel=\"self\" href=\"".$website_url."users/".$username."/atom.xml\"/>\n\t<updated>".$nowa."</updated>\n\t<id>".$website_url."users/".$username."</id>\n\n";
	fwrite($atommeta,$atomstart);

	// if we have posts from this user, show the most recent 25.
	if (post_quantity($posts) > 0) {
		$pst_q = "SELECT * FROM posts WHERE post_status=\"PUBLIC\" AND post_type=\"POST\" AND user_id=\"".$userid."\" ORDER BY post_date DESC LIMIT 25";
 ...
 */
	...
	$rss2meta = fopen($website_url."users/".$username."-rss2.xml", "w") or die("Unable to open or create ".$website_url."users/".$username."-rss2.xml file");
	$rss2start = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n<rss version=\"2.0\">\n<channel>\n\t<title>".$website_name." : ".$username."</title>\n\t<description>"._('The RSS feed of')." @".$username."@".short_url($website_url)."</description>\n\t<link>".$website_url."users/".$username."/</link><pubDate>".$nowp."</pubDate>\n\t<lastBuildDate>".$nowb."</lastBuildDate>\n\n";
	fwrite ($rss2meta,$rss2start);

	// if we have posts, display the most recent ones in a div on the right side of the page
	if (post_quantity($posts) > 0) {
		$rsspst_q = "SELECT * FROM posts WHERE post_status=\"PUBLIC\" AND post_type=\"POST\" AND user_id=\"".$userid."\" ORDER BY post_date DESC LIMIT 25";
